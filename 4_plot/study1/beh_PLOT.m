...
%% 3. Temporal GLM
glmt = [loadfolder 'beh_glm_temp_09-Sep-2024.csv']; 
glmt = readtable(glmt);
% prep for plotting
T    = [glmt.Sglm_pcodexF, glmt.Sglm_scodexF, glmt.Pglm_pcodexF, glmt.Pglm_scodexF];
ind = repelem([1,2],size(T,1)); ind = [ind ind]; 
var = {'Group','Individuation'};
var = var(ind);
ind = repelem([1,2],size(T,1)*2); 
str = {'Self','Partner'}; 
str = str(ind); 
...
g.set_order_options('x',0);
g.set_names('x','Vector','y','Beta (effect size)');
g.draw();
...
saveas(gcf,[outfolder 'beh_glm_temp_' date]);


%% 4. Temporal GLM binned by RT
...
saveas(gcf,[outfolder 'FigS15_beh_glm_temp_rtsplit_' date]);
